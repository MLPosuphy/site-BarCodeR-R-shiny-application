...
const tutorialPreview = {
  poster: `${import.meta.env.BASE_URL}media/tuto-barcoder-test-poster.jpg`,
  video: `${import.meta.env.BASE_URL}media/tuto-barcoder-test.mp4`
};
...
const datasets = [
  { name: "GlobalPatterns", marker: "16S", package: "phyloseq", description: { fr: "Jeu de données environnemental varié, adapté à une première prise en main complète de l’interface.", en: "A varied environmental dataset suited to a first complete tour of the interface." }, file: "GlobalPatterns.RData", size: "425 KB", source: "https://github.com/joey711/phyloseq/blob/master/data/GlobalPatterns.RData", raw: "https://raw.githubusercontent.com/joey711/phyloseq/master/data/GlobalPatterns.RData" },
  { name: "enterotype", marker: "16S", package: "phyloseq", description: { fr: "Objet issu de données du microbiome intestinal humain, utile pour tester l’import et les comparaisons par groupes.", en: "An object based on human gut microbiome data, useful for testing import and group comparisons." }, file: "enterotype.RData", size: "191 KB", source: "https://github.com/joey711/phyloseq/blob/master/data/enterotype.RData", raw: "https://raw.githubusercontent.com/joey711/phyloseq/master/data/enterotype.RData" },
  { name: "soilrep", marker: "16S", package: "phyloseq", description: { fr: "Objet consacré à la reproductibilité de données de microbiome du sol.", en: "An object focused on reproducibility in soil microbiome data." }, file: "soilrep.RData", size: "RData", source: "https://github.com/joey711/phyloseq/blob/master/data/soilrep.RData", raw: "https://raw.githubusercontent.com/joey711/phyloseq/master/data/soilrep.RData" },
  { name: "esophagus", marker: "16S", package: "phyloseq", description: { fr: "Petit objet phyloseq, pratique pour vérifier rapidement l’import et la navigation.", en: "A small phyloseq object convenient for quickly checking import and navigation." }, file: "esophagus.RData", size: "RData", source: "https://github.com/joey711/phyloseq/blob/master/data/esophagus.RData", raw: "https://raw.githubusercontent.com/joey711/phyloseq/master/data/esophagus.RData" },
  { name: "atlas1006", marker: "HITChip", package: "microbiome", description: { fr: "Profils du microbiote intestinal au niveau genre pour 1 006 adultes occidentaux sans complication de santé rapportée, fournis sous forme d’objet phyloseq.", en: "Genus-level gut microbiota profiles from 1,006 Western adults with no reported health complications, provided as a phyloseq object." }, file: "atlas1006.rda", size: "R package data", source: "https://github.com/microbiome/microbiome/blob/master/data/atlas1006.rda", raw: "https://raw.githubusercontent.com/microbiome/microbiome/master/data/atlas1006.rda" },
  { name: "dietswap", marker: "HITChip", package: "microbiome", description: { fr: "Étude longitudinale de deux semaines comparant des groupes africains et afro-américains au cours d’un échange alimentaire, distribuée comme objet phyloseq.", en: "Two-week longitudinal diet-swap study involving African and African American groups, distributed as a phyloseq object." }, file: "dietswap.rda", size: "R package data", source: "https://github.com/microbiome/microbiome/blob/master/data/dietswap.rda", raw: "https://raw.githubusercontent.com/microbiome/microbiome/master/data/dietswap.rda" }
];
...
export default function TutorialsPageV2({ language }: { language: Language }) {
  ...
  const c = language === "fr" ? {
    ...
    result: "Résultat attendu", steps: "Étapes du tutoriel", available: "Tutoriel en préparation", preview: "Aperçu vidéo temporaire", launch: "Tester la lecture vidéo", close: "Fermer le tutoriel", videoNote: "Vidéo de démonstration — à remplacer par le tutoriel final",
    dataK: "Objets phyloseq publics", dataT: "Télécharger des jeux de données prêts à importer.", dataP: "Ces objets publics sont distribués dans les packages phyloseq et microbiome. Ils permettent de tester rapidement l’import et les fonctions de BarCodeR sans utiliser de données personnelles.",
    download: "Télécharger l’objet", source: "Voir la source officielle", format: "Format", package: "Package R"
  } : {
    ...
    result: "Expected result", steps: "Tutorial steps", available: "Tutorial in preparation", preview: "Temporary video preview", launch: "Test video playback", close: "Close tutorial", videoNote: "Demonstration video — to be replaced by the final tutorial",
    dataK: "Public phyloseq objects", dataT: "Download datasets ready to import.", dataP: "These public objects are distributed in the phyloseq and microbiome packages. They provide a quick way to test BarCodeR import and features without using personal data.",
    download: "Download object", source: "View official source", format: "Format", package: "R package"
  };
