...
export default function InstallerPageV2({ language }: { language: Language }) {
  ...
  const c = language === "fr" ? {
    ...
    packagingK: "Une application autonome", packagingT: "Tout ce dont BarCodeR a besoin est déjà intégré.", packagingP: "Chaque version est préparée et testée avant sa publication. Le dossier téléchargé réunit l’application et son environnement : il suffit de le conserver complet et d’ouvrir BarCodeR.",
    ...
    tutorials: "Ouvrir les tutoriels", directDownload: "Télécharger BarCodeR", pendingDownload: "Archive de distribution à intégrer", pendingP: "Le bouton de téléchargement sera activé dès que l’archive .tar de BarCodeR sera ajoutée au site."
  } : {
    ...
    packagingK: "A standalone application", packagingT: "Everything BarCodeR needs is already included.", packagingP: "Each version is prepared and tested before publication. The downloaded folder brings the application and its environment together: simply keep it complete and open BarCodeR.",
    ...
    tutorials: "Open tutorials", directDownload: "Download BarCodeR", pendingDownload: "Distribution archive to be added", pendingP: "The download button will be enabled as soon as the BarCodeR .tar archive is added to the website."
  };

  return <main className="installer-v2-page">
    <section className="installer-v2-hero"><div className="page-width"><header><Eyebrow>{c.k}</Eyebrow><h1>{c.title}</h1><p>{c.p}</p><div className="installer-v2-hero-actions">{downloadUrl ? <a className="button primary" href={downloadUrl} download>{c.directDownload}<span>↓</span></a> : <span className="installer-v2-download-pending" aria-disabled="true"><b>{c.pendingDownload}</b><small>{c.pendingP}</small></span>}<a className="button secondary" href="#installer-process">{language === "fr" ? "Voir l’installation" : "View installation"}<span>↓</span></a></div></header><aside><div className="installer-v2-app-icon"><span>B</span><i /></div><small>{c.release}</small><h2>BarCodeR <em>v{appVersion}</em></h2><dl><div><dt>{c.system}</dt><dd>Windows</dd></div><div><dt>{c.status}</dt><dd>EXE</dd></div><div><dt>{c.ready}</dt><dd>✓</dd></div></dl></aside></div></section>
